...
%% Example: SOM-based crack segmentation on a brick wall image
% Input image should be an RGB photo named 'brick.png' in the same folder.
I = imread('brick.png');
...
% (Optional ablation examples – comment/uncomment as needed)
% input = double([...]);  % etc.

%% Train SOM (5 units for brick cracks + backgrounds)
nClasses = 5;
net      = selforgmap(nClasses);
[net, tr] = train(net, input);
op        = vec2ind(net(input));
weight    = getwb(net);
[b, Iw, Lw] = separatewb(net, weight); %#ok<ASGLU>
z         = dist(Iw{1,1}, input);
euclidean = min(z);                     %#ok<NASGU>
[ro, col] = find(z == euclidean);       %#ok<NASGU>
